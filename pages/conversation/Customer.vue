    ...
    >
      <view class="nav" v-if="conversationInfo.conversation.type == 2">
        ...
        <view @click="ruletost" class="rule">
          <image
            src="/assets/images/ic_error.svg"
            style="display: block"
          ></image>
        </view>
      </view>
      ...
      <view class="message-list-container" @click="fatherClick">
        ...
        >
          <view
            v-for="(
              message, index
            ) in sharedConversationState.currentConversationMessageList"
...
export default {
  ...
  mounted() {
	...
	console.log("xxxx", this.sharedConversationState.currentConversationMessageList)
  ...
  computed: {
    ...
    lastMessageId() {
      if (
        this.conversationInfo &&
        this.conversationInfo.lastMessage &&
