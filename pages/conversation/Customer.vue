...
export default {
	...
	created() {
		 ...
		 // console.log('99我进来了2211',userInfo)
		this.getMessage()
	},
    methods: {
		getMessage(){
			let users = [{uid:getItem("chatId")}]
			store.createConversation(users, (conversation) => {
				
				store.getMessages(conversation, getItem("chatId"), true, '', (msgs)=>{
			        
					console.log(msgs,"哈哈哈11");
					if(msgs.length>0){
						this.sharedConversationState.currentConversationMessageList = msgs
						uni.hideLoading({})
					}else{
						this.getMessage()
					}
